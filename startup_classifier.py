...
class Key_words:
    def __init__(self):
        self.slov = ['Transport', 'Public transport', 'Car', 'Bus', 'Train', 'Trolley', 'Trolleybus',
                     ...
                     'Machine', 'Autopilot', 'Taxi', 'Robotaxi', 'Robobus', 'Robotrain', 'Robocar', 'Transportation',
                     'Mobility', 'Future transportation', 'Scooter', 'Electric scooter',
                     'Bike', 'Bycicle', 'Electric bycicle', 'Micromobility', 'Company', 'Electric', 'Public',
...
class Startup_classifier:
    ...
    def predict(self, txt):
        if 0.15 * self.keywords_classifier.predict(txt) + 0.85 * self.classifier_regres.predict_proba([txt])[0][
            1] >= 0.5:
...
startup_test = Startup_classifier()
print(startup_test.predict(txt))
